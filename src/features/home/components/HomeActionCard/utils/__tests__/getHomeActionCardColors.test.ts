...
import { getHomeActionCardColors } from "../getHomeActionCardColors";

describe("getHomeActionCardColors", () => {
  it.each([
    [
      "hero",
      {
    ...
    "returns the expected colors for %s variant",
    (variant, expectedColors) => {
      expect(getHomeActionCardColors(variant)).toEqual(
        expectedColors,
      );
    },
  );
